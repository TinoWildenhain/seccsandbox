...
SYSCALLS = {
    ...
    207: "io_destroy", 208: "io_getevents", 209: "io_submit", 210: "io_cancel",
    ...
    329: "pkey_mprotect", 330: "pkey_alloc", 331: "pkey_free", 332: "statx",
    ...
    432: "fsmount", 433: "fspick", 434: "pidfd_open", 435: "clone3",
...
def decode_audit_line(line):
    ...
    if syscall_match:
        ...
        print(f"SECCOMP VIOLATION: Process '{comm}' (PID {pid}) blocked syscall: {syscall_name} ({syscall_num})")
        return True
    return False

if __name__ == "__main__":
    if len(sys.argv) > 1:
        # Decode specific line
        decode_audit_line(sys.argv[1])
    else:
        # Read from stdin
        for line in sys.stdin:
            if "type=1326" in line:  # SECCOMP audit type
                decode_audit_line(line.strip())
